...
public class CompareLinearAndNonLinearImages extends AbstractTiffProcessor {
	...
	private double[][] cameraToXYZ_D50(double weight, double[] cameraNeutral) {
		double[][] inverseCC = inverse(vector2Matrix(SRATIONAL.asDoubleArray((SRATIONAL[]) data.get(Tag.CameraCalibration1))));
		return
			multiply(
				multiply(
					weightedAverage(
						vector2Matrix(SRATIONAL.asDoubleArray((SRATIONAL[]) data.get(Tag.ForwardMatrix1))),
						vector2Matrix(SRATIONAL.asDoubleArray((SRATIONAL[]) data.get(Tag.ForwardMatrix2))),
						weight
					),
					inverse(asDiagonalMatrix(Math.multiply(inverseCC, cameraNeutral)))
				),
				inverseCC
			);
	}
